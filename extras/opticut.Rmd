...
(note the small sample size where this really can be a problem)
...
Y <- rpois(n, lam)
o <- opticut(Y ~ 1, strata=x0, dist="poisson", comb="rank")
summary(o)
...
```{r}
uc <- uncertainty(o, 1, type="a")
i <- 1
summary(uc[[i]])
plot(density(uc[[i]]$I))
data.frame(table(uc[[i]]$best))[order(table(uc[[i]]$best)),]
```

...
rownames(spp) <- gr
spp
summary(mod <- opticut(spp ~ 1, strata=gr, dist="poisson", comb="all"))
summary(mod <- opticut(spp ~ 1, strata=gr, dist="poisson", comb="rank"))
```
...
```{r}
uc <- uncertainty(o, 1, type="asymp")
summary(uc[[1]])
plot(density(uc[[1]]$I))
data.frame(table(uc[[1]]$best))[order(table(uc[[1]]$best)),]
```
...
* add HPC option to uncertainty calculations
* build the package: R CMD check cleanup
* Add SV N-mix and offset examples

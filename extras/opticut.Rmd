...
It only takes a classification vector with $n > 1$ partitions.

`nComb` returns a 'contrast' matrix corresponding to
...
The number of such possible combinations is $2^{n-1} - 1$.
...
* 3 fake species from Legendre
  - discuss differences
  - explain components of the analysis
  - lay out hypothesis in evidential terms (Ecology p-value section?)
* Dolina data set, as in Com Ecol
  - see effect of confounding: split, effect size, w, LR
* Power calculation with confounding

## Challenges

* Explain as model selection, not p-value / testing
* Provide general hierarchical description of the problem & model
  (w, penalty involved)
* Dune: releve data, ordinal ?
* Mites, dolina, EA's bugs: count (can be 0/1)

## Extensions

* presence only data
* detection error (N-mix, etc, offsets)
...
opticut1(Y1, X, as.factor(x0), dist="poisson")
opticut1(Y2, X, Z, dist="poisson")
opticut1(Y2, X, as.factor(x0), dist="poisson")
summary(mod <- opticut(Y ~ x2, strata=x0, dist="poisson"))
## comparing rank based vs all combinations
r1 <- opticut(Y ~ x2, strata=x0, dist="poisson", comb="rank")
r2 <- opticut(Y ~ x2, strata=Z[,1:4], dist="poisson", comb="all")

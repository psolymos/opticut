...
rownames(xx) <- rownames(summary(x1)$summary)
xx
ocoptions(oc)
```
...
aa=t(sapply(1:ncol(y), function(i) zi_beta_fun(yy[,i], data.frame(matrix(1, nrow(yy), 1)))$coef))

yy <- rbind(y,y,y,y,y)
gr <- rep(varechem$grazing, 5)
x <- opticut(yy ~ 1, strata=gr, dist=zi_beta_fun)
summary(x)
plot(x, cut=-Inf)

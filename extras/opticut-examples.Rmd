...
```{r}
oc_sim_K2 <- 
function(b1=0.5, b2=0, b3=0, b4=0, mu0=10, n=100, pos=TRUE)
{
    K <- 2
    n <- 2*(n %/% 2)
    i <- 1:n
    ...
    #Conf <- rep(seq(-0.5*diff(mu), 0.5*diff(mu), length=n/2), 2)
...
est_fun <- function(ii, R=999, level=0.95) {
    dat <- oc_sim_K2(
        b1=vals$b1[ii], 
        b2=vals$b2[ii], 
        b3=vals$b3[ii], 
    ...
    m1 <- opticut(Y ~ x, dat, strata=h1)$species[[1]]
    ## IndVal
...
dat <- oc_sim_K2(b1=0.9, b2=-0.5, b3=1, b4=0, mu0=10, n=100)
plot(Y ~ x, dat)
m0 <- opticut(Y ~ 1, dat, strata=h1)

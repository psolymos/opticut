...
library(parallel)
cl <- makeCluster(4)
clusterExport(cl, c("est_fun", "est_fun1", "oc_sim_K2", "B"))
clusterEvalQ(cl, library(opticut))
clusterEvalQ(cl, library(indicspecies))

## contrast (I)
res1 <- parLapply(cl, seq(0.1, 0.9, by=0.1), function(z) est_fun(B=B, b1=z))

## confounding
res2 <- lapply(seq(0, 1, by=0.1), function(z) est_fun(B=B, b2=z))

## noise
res3 <- lapply(seq(0.1, 1, by=0.1), function(z) est_fun(B=B, b3=z))

## mixing
res4 <- lapply(seq(0, 1, by=0.1), function(z) est_fun(B=B, b4=z))
...
library(indicspecies)
ii <- 40 # row from vals
R <- 999 # no. iter for p-values (each run)
B <- 100 # replicates




dat <- oc_sim_K2(b1=0.5, b2=0, b3=0, b4=0, mu0=10, n=100)
plot(Y ~ x, dat)
m0 <- opticut(Y ~ 1, dat, strata=h1)
m1 <- opticut(Y ~ x, dat, strata=h1)
m0$species[[1]]
m1$species[[1]]

## Noise
dat <- oc_sim_K2(b1=0.9, b2=0, b3=1, b4=0, mu0=10, n=100)
plot(Y ~ x, dat)
m0 <- opticut(Y ~ 1, dat, strata=h1)
m1 <- opticut(Y ~ x, dat, strata=h1)
m0$species[[1]]
...
## Noise + Confounding
## mu0 and mu1 are measured at 0 level for X
dat <- oc_sim_K2(b1=0.9, b2=1, b3=1, b4=0, mu0=10, n=100)
plot(Y ~ x, dat)
m0 <- opticut(Y ~ 1, dat, strata=h1)
m1 <- opticut(Y ~ i, dat, strata=h1) # negative m0
m1 <- opticut(Y ~ x, dat, strata=h1) # centering helps
m1 <- opticut(Y ~ I(i-40), dat, strata=h1) # centering helps
m0$species[[1]]
m1$species[[1]]

## Noise + Misclassification
dat <- oc_sim_K2(b1=0.9, b2=0, b3=1, b4=1, mu0=10, n=100)
plot(Y ~ x, dat, col=dat$h1)
m0 <- opticut(Y ~ 1, dat, strata=h1)
m1 <- opticut(Y ~ x, dat, strata=h1)
m0$species[[1]]
m1$species[[1]]

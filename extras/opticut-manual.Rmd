...
data(dolina)
dolina$samp$stratum <- as.integer(dolina$samp$stratum)
inew <- 1:nrow(dolina$samp) %in% sample(nrow(dolina$samp), 50)
Y <- dolina$xtab
...
y <- Y[!inew,]
ynew <- Y[inew,]
#o <- opticut(y ~ stratum + lmoist + method, data=x, strata=mhab, dist="binomial")
...
cm <- table(pred=cal$gnew, true=dolina$samp[inew,"mhab"])

...
Stratigraphy example using Beta distribution: the only limitation of the Beta distribution in this context is that boundary values (0 and 1) are not part of the support, so a tiny value (here 0.0001) is used instead. It is explained later how to define a zero-inflated Beta distribution.

The data we are using is from the **rioja** package:
pollen stratigraphic data from Abernethy Forest, Scotland, spanning approximately 5500--12100 BP (from Birks & Mathews 1978, New Phytologist 80, 455--84).
...
The following data set is from the **optpart** package:
...
Presence only data is a result from animal movement studies, or based on museum specimens. Here is an example of Mountain Goat telemetry data that were collected in the Coast Mountains of northwest British Columbia, Canada, as described in Lele and Keim (2006, Ecology 87, 3021-3028).

Multiple species analysis is not possible doe to different number of used (presence) points for each species. It leads to a ragged-array format which is not straightforward to supply through the formula interface. The analysis of such presence-only data is also subject to conditions as explained by Solymos and Lele (2016, Methods in Ecology and Evolution, 7, 196-205).
...
We implement a zero-inflated Beta distribution:
...
o3 <- opticut(sveg ~ 1, strata=elev, dist=zi_beta_fun)
plot(o3, sort=1)
```
...
Lele, Subhash R., Jonah L. Keim and Peter Solymos (2016).
...
Oksanen, Jari, F. Guillaume Blanchet, Michael Friendly, Roeland
...
Zeileis, Achim, Christian Kleiber, Simon Jackman (2008).
